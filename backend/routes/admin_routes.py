...
@admin_required
def upload_excel():
    """Upload Excel file for analysis (admin only)"""
    try:
        ...
        if 'error' not in summary:
            ...
            # Financial analysis
            financial_analysis = processor.analyze_financial_data()
            
            if 'error' not in financial_analysis:
                financial_result = AnalysisResult(
                    excel_file_id=excel_file.id,
                    result_type='financial',
                    result_data=financial_analysis
                )
                
                db.session.add(financial_result)
            
            # Commission analysis if applicable
            commission_analysis = processor.extract_commission_data()
            
            if 'error' not in commission_analysis:
                commission_result = AnalysisResult(
                    excel_file_id=excel_file.id,
                    result_type='commission',
                    result_data=commission_analysis
                )
                
                db.session.add(commission_result)
            
            # Mark analysis as complete
            excel_file.analysis_complete = True
