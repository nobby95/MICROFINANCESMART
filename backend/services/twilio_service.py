...
def send_loan_approval_notification(to_number, client_name, amount, loan_id):
    """Send loan approval notification"""
    message = f"Hello {client_name}, your loan application (ID: {loan_id}) for ${amount:,.2f} has been approved!"
    return send_message(to_number, message)

def send_payment_reminder(to_number, client_name, amount, due_date):
    """Send payment reminder"""
    message = f"Hello {client_name}, this is a reminder that a payment of ${amount:,.2f} is due {due_date}."
    return send_message(to_number, message)

def send_payment_confirmation(to_number, client_name, amount, loan_id):
    """Send payment confirmation"""
    message = f"Hello {client_name}, we've received your payment of ${amount:,.2f} for loan {loan_id}. Thank you!"
    return send_message(to_number, message)
