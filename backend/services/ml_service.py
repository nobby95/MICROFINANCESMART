...
class MicrofinanceML:
    ...
    def analyze_loan_risk(self, loan_data):
        """Analyze loan application risk"""
        try:
            # Extract features
            features = np.array([
                loan_data.get('amount', 0),
                loan_data.get('term_months', 0),
            ...
            risk_score = np.clip(
                (features_scaled[0][0] * 0.4 + 
                 features_scaled[0][1] * 0.3 + 
                 features_scaled[0][2] * 0.3), 0, 1)
    ...
    def predict_loan_amount(self, client_data):
        """Predict suitable loan amount"""
        try:
            # Extract features
            features = np.array([
                client_data.get('monthly_income', 0),
                client_data.get('credit_score', 0),
                client_data.get('existing_loans', 0)
            ]).reshape(1, -1)
            
            # Scale features
            features_scaled = self.scaler.fit_transform(features)
            
            # Simple amount prediction
            predicted_amount = np.clip(
                features_scaled[0][0] * 10000 * (
                    1 + features_scaled[0][1] * 0.5 - 
                    features_scaled[0][2] * 0.3
                ), 1000, 50000)
